...
import { deleteTestSessionOnly } from "@/lib/admin/deleteSessionData";
...
/**
 * 刪除此筆測驗（及 answers / session_questions / recommendations 等關聯），保留學生與家長。
 */
export async function DELETE(
  ...
  const supabase = createAdminSupabaseClient();
  const r = await deleteTestSessionOnly(supabase, sessionId);
  if (!r.ok) {
    if (r.message === "找不到此筆測驗") {
